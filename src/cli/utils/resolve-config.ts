...
function assertBoolean(
  ...
  if (typeof value !== "boolean") {
    throw new Error(
      `Field ${field} is missing or invalid in the TSera configuration.`,
    );
  }
}
...
function assertStringArray(
  ...
  if (!Array.isArray(value)) {
    throw new Error(
      `Field ${field} must be an array of strings in the TSera configuration.`,
    );
  }
 ...
 */
function assertPathsConfig(
  value: unknown,
): asserts value is TseraConfig["paths"] {
  ...
  if ((paths.entities as string[]).length === 0) {
    throw new Error(
      "The TSera configuration must declare at least one entity path.",
    );
  }
...
function assertDbConfig(value: unknown): asserts value is TseraConfig["db"] {
  ...
  if (dialect !== "postgres" && dialect !== "mysql" && dialect !== "sqlite") {
    throw new Error(
      "Unsupported database dialect. Expected postgres, mysql, or sqlite.",
    );
  }
  ...
  if (dialect === "postgres") {
    ...
    if (
      db.ssl !== undefined && db.ssl !== "disable" && db.ssl !== "prefer" &&
      db.ssl !== "require"
    ) {
      throw new Error(
  ...
  if (dialect === "mysql") {
    ...
    if (db.ssl !== undefined && typeof db.ssl !== "boolean") {
      throw new Error(
        "MySQL SSL configuration must be a boolean when provided.",
      );
    }
    if (db.file !== undefined) {
  ...
  if (
    db.file === undefined || typeof db.file !== "string" || db.file.length === 0
  ) {
  ...
  if (
    db.urlEnv !== undefined &&
    (typeof db.urlEnv !== "string" || db.urlEnv.length === 0)
  ) {
...
function assertDeployConfig(
  value: unknown,
): asserts value is TseraConfig["deploy"] {
  if (!value || typeof value !== "object") {
  ...
  if (
    ...
    throw new Error(
      "Unsupported deploy target. Expected deno_deploy, cloudflare, or node_pm2.",
    );
  }
