...
/**
 * Generates env.config.ts file with schema definitions.
 *
 * @param config - Generation configuration.
...
export function generateEnvSchema(config: EnvGenerationConfig): string {
  ...
  const lines: string[] = [
    'import { defineEnvSchema } from "tsera/core/secrets";',
    "",
    "/**",
  ...
  if (db.dialect === "postgres" || db.dialect === "mysql") {
    ...
    lines.push(`    type: "${db.dialect === "postgres" ? "string" : "boolean"}",`);
    lines.push("    required: false,");
    if (db.dialect === "postgres") {
      lines.push('    default: "prefer",');
    } else {
      lines.push("    default: false,");
    }
    lines.push('    description: "SSL mode for database connection",');
    lines.push("  },");
  }
  ...
  lines.push("});");
  lines.push("");
  lines.push("export type Env = typeof envSchema;");
